...
class FaceTrackingSystem:
    def __init__(self, api_config):
        ...
        # *** MODIFIED: Load camera configs into both a list and a dictionary ***
    ...
    def _initialize_faiss(self):
        if self.embeddings.size > 0: # FIX: Check if the numpy array is not empty
        ...
        while not self.shutdown_flag.is_set():
            try:
                ...
                if current_time - self.last_faces_reload > 300:
                    ...
                    if new_embeddings.size > 0: # FIX: Check if numpy array is not empty
                ...
                if width > 960:
                    ...
                    new_width = int(width * scale_factor)
                    new_height = int(height * scale_factor)
                    # --- FIX: Corrected call to cv2.resize ---
                    enhanced_frame = cv2.resize(enhanced_frame, (new_width, new_height))
                faces = self.apps[gpu_id].get(enhanced_frame)
                if scale_factor != 1.0:
                    for face in faces:
    ...
    def _compute_embedding_similarity(self, embedding: np.ndarray) -> Tuple[str, float]:
        ...
        with self.faiss_index_lock:
            ...
            try:
                ...
                # --- FIX: Corrected variable names from faiss search ---
    ...
    def _temporal_smoothing(self, identity: str, score: float, camera_id: int) -> Tuple[str, float]:
        ...
        if track_key not in self.track_identities[camera_id]: # FIX: Access per-camera dictionary
        ...
        if len(track_data['votes']) >= 3:
            ...
            if identity_counts:
                ...
                return best_identity[0], min(best_identity[1], float(avg_score)) # FIX: Cast avg_score to float
        ...
        try:
            if hasattr(face, 'landmark_2d_106'):
                ...
                if landmarks is not None and len(landmarks) > 0:
                    ...
                    # --- FIX: Explicitly cast to float to match return type hint ---
    ...
    def _compute_sharpness_score(self, face, bbox) -> float:
        try:
            if hasattr(face, 'embedding'):
                ...
                return float(normalized_var) # FIX: Cast to float
            return 0.5
        except:
            return 0.5
    ...
    def _compute_face_angle_score(self, face) -> float:
        try:
            if hasattr(face, 'pose'):
                ...
                return float(max(0.0, 1.0 - angle_penalty)) # FIX: Cast to float
            return 0.8
        except:
            return 0.8
    ...
    def process_camera(self, camera_config: Dict):
        ...
        print(f"[CAMERA] Starting video stream processing for '{cam_name}' (ID: {cam_id}).")
        while not self.shutdown_flag.is_set():
            cap = None
            try:
                ...
                while not self.shutdown_flag.is_set():
                    ...
                    # Get processed face data and store it
                    try:
                        ...
                        # If no new faces, clear old tracking data to avoid stale boxes
                        with self.tracking_data_locks[cam_id]:
                ...
                if cap and cap.isOpened():
                    cap.release()
        print(f"[CAMERA] Stopped video stream processing for '{cam_name}' (ID: {cam_id}).")
    ...
    def start_multi_camera_tracking(self):
        print(f"[SYSTEM] Starting tracking on {len(self.camera_configs)} configured camera(s)...")
        for camera_config in self.camera_configs:
            ...
            time.sleep(1)
        print("[SYSTEM] All camera processing threads have been started.")
    ...
    def shutdown(self):
        ...
        for cam_id in self.frame_queues:
            try:
                self.frame_queues[cam_id].put(None, timeout=0.1)
            except queue.Full:
                pass
